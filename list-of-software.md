...
[Linux Mint](https://linuxmint.com) version 21 is a Linux distribution, typically known as the distribution that works out-of-the-box when it comes to proprietary multimedia codecs. As for the moment, I only use Linux Mint as a virtual machine (VM) to create and run a local installation of a TeX distribution which is TeX Live. In that I also run TeXstudio, which is a LaTeX editor.
